...
%%
%%% 2. Dane
Pn = 13000;  %moc znamionowa [W]
Un = 230;   %napiêcie znamionowe [V]
In = 67.2;    %pr¹d znamionowy [A]
Nn = 900;  %prêdkoœæ znamionowa [obr/min]
R = 0.423;  %rezystancja twornika [Ohm]
L = 0.0021;   %indukcujnosc twornika [H]
Js = 0.42;  %moment bezwladnosci [kg*m^2]
%%
%%
%%% 3. Prze³¹cznia uk³adu
...
J = 9*Js;   %zastêpszy moment bezw³adnoœci
...
Uz0 = lambdan*In*Y*B1/(B1-beta)*0.87;    %ograniczenie Uz0 < 10V
...
%% PUNKT 1 Z INSTUKCJI "projekt2015"
%%
%%% 6. Odpowiedzi modelów w postaci transmitancji na skok impilsowy
sim('projekt1_KUS_sim');
figure()
grid on
hold on
title('GIU - odpowiedŸ skokowa pr¹du twornika');
xlabel('Czas [s]');
ylabel('Pr¹d [I]');
ylim([0 1800]);
plot(GUI_imp.time,GUI_imp.signals.values(:,1),'LineWidth',2);
plot(time,Imax,'r','LineWidth',2);

figure()
grid on
hold on
title('GIU pochodna - odpowiedŸ skokowa pochodnej pr¹du twornika');
xlabel('Czas [s]');
ylabel('Pochodna pr¹du [I/s]');
ylim([-1e5, 2e5]);
plot(GUI_der_imp.time,GUI_der_imp.signals.values(:,1),'LineWidth',2);
plot(time,p*In+0*time,'r','LineWidth',2);
...
% % ylim([0 10]);
% % plot(GomegaM_imp.time,GomegaM_imp.signals.values(:,1),'LineWidth',2);


figure()
grid on
hold on
title('Prêdkoœæ k¹towa omega');
xlabel('Czas [s]');
ylabel('Pulsacja [rad/s]');
ylim([0 100]);
plot(GomegaU_imp.time,GomegaU_imp.signals.values(:,1),'LineWidth',2);
plot(time,omegan+0*time,'r','LineWidth',2);
...
% % ylabel('Pr¹d [I]');
% % ylim([0 500]);
% % plot(GIM_imp.time,GIM_imp.signals.values(:,1),'LineWidth',2);
% % plot(time,Imax,'r','LineWidth',2);

%% PUNKT 1 Z INSTUKCJI "projekt2015"
%%  
%%% 7. Rysowanie przebiegów symulacyjnych - uk³ad analogowy
% ¯eby pr¹d nie przekracza³ granicy ustawiono Uz0 = 0.85
...
for x = 1:2
    for p = 1:3
        ...
        elseif (p == 3)
            Top = 2;    %opóŸnie za³¹czenie momentu (za³¹czenie udarowe)
        end;
        sim('projekt1');
...
for x = 1:2
    ...
    if(x == 1)
        ...
        xlim([0 6]);
    elseif (x == 2)
        plot(us.time,us.signals.values(:,1),'g')
        xlim([0 6]);
    end;
    title('Napiêcie silnika');
    xlabel('Czas [s]');
    ylabel('Napiêcie [V]');
    if(x == 2)
        legend('prawid³owy czas','nieprawid³owy czas');
    end;
    ...
    if(x == 1)
        ...
        xlim([0 7]);
    elseif (x == 2)
        plot(dltpipi.time,dltpipi.signals.values(:,1),'g')
        xlim([0 7]);
    end;
    plot(Imax.time,Imax.signals.values(:,1),'r');
    title('Pr¹d silnika');
    ...
    ylabel('Pr¹d [A]');
    if(x == 2)
        legend('prawid³owy czas','pr¹d maksymalny','nieprawid³owy czas');
    end;
    ...
    if(x == 1)
        ...
        xlim([0 6]);
    elseif (x == 2)
        plot(domegapipi.time,domegapipi.signals.values(:,1),'g')
        xlim([0 6]);
    end;
    title('Pulsacja silnika');
    xlabel('Czas [s]');
...
for x = 1:2
    for p = 1:3
        ...
        title('P silnika');
        xlabel('Czas [s]');
        ylabel('Prêdkoœæ obrotowa [rad/s]');
    end;
end;
regpred = 1;
Top = 0;
